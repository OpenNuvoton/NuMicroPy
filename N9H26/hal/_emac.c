 ...
 * @file     emac.c
 * @version  V1.00
 * @brief    N9H26 EMAC driver source file
 *
...
/**
  ...
  * @return None
  */
#define EMAC_TRIGGER_RX() do{EMAC->RSDR = 0UL;}while(0)
...
/**
  ...
  * @return None
  */
#define EMAC_TRIGGER_TX() do{EMAC->TSDR = 0UL;}while(0)
...
static void EMAC_MdioWrite(uint32_t u32Reg, uint32_t u32Addr, uint32_t u32Data)
{
    ...
    EMAC->MIIDA = u32Reg | (u32Addr << 8) | EMAC_MIIDA_BUSY_Msk | EMAC_MIIDA_WRITE_Msk | EMAC_MIIDA_MDCON_Msk;
...
static uint32_t EMAC_MdioRead(uint32_t u32Reg, uint32_t u32Addr)
{
    ...
    EMAC->MIIDA = u32Reg | (u32Addr << EMAC_MIIDA_PHYAD_Pos) | EMAC_MIIDA_BUSY_Msk | EMAC_MIIDA_MDCON_Msk;
...
static void EMAC_PhyInit(void)
{
    ...
    while(!(EMAC_MdioRead(PHY_STATUS_REG, EMAC_PHY_ADDR) & PHY_STATUS_LINK_VALID))
    {
        ...
        {
            EMAC->MCMDR &= ~EMAC_MCMDR_OPMOD_Msk;
            EMAC->MCMDR &= ~EMAC_MCMDR_FDUP_Msk;
            break;
        }
    }
...
static void EMAC_TxDescInit(void)
{
    ...
    EMAC->TXDLSA = (uint32_t)&tx_desc[0];
...
static void EMAC_RxDescInit(void)
{
    ...
    /* Get Frame descriptor's base address. */
    EMAC->RXDLSA = (uint32_t)&rx_desc[0];
...
void EMAC_Open(uint8_t *pu8MacAddr)
{
    ...
    EMAC->MCMDR = EMAC_MCMDR_SPCRC_Msk |
                EMAC_MCMDR_ENRMII_Msk;

    /* Accept packets for us and all broadcast and multicast packets */
    EMAC->CAMCMR =  EMAC_CAMCMR_ECMP_Msk |
                    EMAC_CAMCMR_AMP_Msk |
                    EMAC_CAMCMR_ABP_Msk;
...
uint32_t EMAC_RecvPkt(uint8_t *pu8Data, uint32_t *pu32Size)
{
    ...
    reg = EMAC->MISTA;
    EMAC->MISTA = reg & 0xFFFFUL;  /* Clear all RX related interrupt status */

    if (reg & EMAC_MISTA_RXBERR_Msk)
    {
...
//    EMAC->MISTA = reg & (0xFFFF0000UL & ~EMAC_MISTA_TSALMIF_Msk);
    EMAC->MISTA = reg & (0xFFFF0000UL);


    if (reg & EMAC_MISTA_TXBERR_Msk)
    {
        /* Bus error occurred, this is usually a bad sign about software bug and will occur again... */
        while(1) {}
    }
...
uint32_t EMAC_SendPktDoneTS(uint32_t *pu32Sec, uint32_t *pu32Nsec)
{
    ...
    reg = EMAC->MISTA;
    /* Clear Tx interrupt flags */
//    EMAC->MISTA = reg & (0xFFFF0000UL & ~EMAC_INTSTS_TSALMIF_Msk);
    EMAC->MISTA = reg & (0xFFFF0000UL);
