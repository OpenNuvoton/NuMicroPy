...
typedef enum {
    PLANAR_PIXFORMAT_INVALID = 0,
    PLANAR_PIXFORMAT_YUV422,
    PLANAR_PIXFORMAT_YUV420,
    PLANAR_PIXFORMAT_YUV420MB,	//for H264 encode source
} planar_pixformat_t;
...
typedef bool (*streaming_cb_t)(image_t *planar_image, image_t *packet_image);
...
//    GPIO_TypeDef *vsync_gpio;   // VSYNC GPIO output port.
	...
	planar_pixformat_t pixformat_planar; // Planar pipe's pixel format
    pixformat_t pixformat_packet;      // Packet pipe's pixel format
    framesize_t framesize_planar;      // Frame size
    framesize_t framesize_packet;      // Frame size
...
//    cambus_t bus;               // SCCB/I2C bus.
    ...
    int  (*snapshot)            (sensor_t *sensor, image_t *planar_image, image_t *packet_image, streaming_cb_t streaming_cb);
} sensor_t;
...
int sensor_snapshot(sensor_t *sensor, image_t *planar_image, image_t *packet_image, streaming_cb_t streaming_cb);
