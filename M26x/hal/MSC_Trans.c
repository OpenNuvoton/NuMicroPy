...
static uint32_t MassBlock[MASS_BUFFER_SIZE / 4];
static uint32_t Storage_Block[STORAGE_BUFFER_SIZE / 4];
...
static void MSC_ReadMedia(uint32_t u32Addr, uint32_t u32Size, uint8_t *pu8Buffer)
{
    ...
    g_STORIF_sFlash.pfnGetInfo(&sStorInfo, NULL);
	...
	if(u32SectorAlignAddr != u32Addr){
		...
		g_STORIF_sFlash.pfnReadSector(pu8TempBuf, u32SectorAlignAddr / sStorInfo.u32SectorSize, 1, NULL);
	...
	g_STORIF_sFlash.pfnReadSector(pu8Buffer, u32Addr / sStorInfo.u32SectorSize, i32EachReadLen / sStorInfo.u32SectorSize, NULL);
	...
	if(u32Size > 0){
		...
		g_STORIF_sFlash.pfnReadSector(pu8TempBuf, u32Addr / sStorInfo.u32SectorSize, 1, NULL);
...
static void MSC_WriteMedia(uint32_t u32Addr, uint32_t u32Size, uint8_t *pu8Buffer)
{
	...
	if(u32SectorAlignAddr != u32Addr){
		...
		g_STORIF_sFlash.pfnWriteSector(pu8TempBuf, u32SectorAlignAddr / sStorInfo.u32SectorSize, 1, NULL);
	...
	g_STORIF_sFlash.pfnWriteSector(pu8Buffer, u32Addr / sStorInfo.u32SectorSize, i32EachWriteLen / sStorInfo.u32SectorSize, NULL);
	...
	if(u32Size > 0){
		...
		g_STORIF_sFlash.pfnWriteSector(pu8TempBuf, u32Addr / sStorInfo.u32SectorSize, 1, NULL);
	}
...
static void MSC_Write(void)
{
    ...
    if (g_u32Length > EP3_MSC_MAX_PKT_SIZE)
    {
        if (USBD_GET_EP_BUF_ADDR(EP3) == g_u32BulkBuf0)
        {
            USBD_SET_EP_BUF_ADDR(EP3, g_u32BulkBuf1);
            USBD_SET_PAYLOAD_LEN(EP3, EP3_MSC_MAX_PKT_SIZE);
            USBD_MemCopy((uint8_t *)g_u32Address, (uint8_t *)((uint32_t)USBD_BUF_BASE + g_u32BulkBuf0), EP3_MSC_MAX_PKT_SIZE);
        }
        else
        {
            USBD_SET_EP_BUF_ADDR(EP3, g_u32BulkBuf0);
            USBD_SET_PAYLOAD_LEN(EP3, EP3_MSC_MAX_PKT_SIZE);
            USBD_MemCopy((uint8_t *)g_u32Address, (uint8_t *)((uint32_t)USBD_BUF_BASE + g_u32BulkBuf1), EP3_MSC_MAX_PKT_SIZE);
        }

        g_u32Address += EP3_MSC_MAX_PKT_SIZE;
        g_u32Length -= EP3_MSC_MAX_PKT_SIZE;
...
)
{
    ...
    USBD->STBUFSEG = SETUP_MSC_BUF_BASE;

    /*****************************************************/
    /* EP0 ==> control IN endpoint, address 0 */
    USBD_CONFIG_EP(EP0, USBD_CFG_CSTALL | USBD_CFG_EPMODE_IN | 0);
    /* Buffer range for EP0 */
    USBD_SET_EP_BUF_ADDR(EP0, EP0_MSC_BUF_BASE);

    /* EP1 ==> control OUT endpoint, address 0 */
    USBD_CONFIG_EP(EP1, USBD_CFG_CSTALL | USBD_CFG_EPMODE_OUT | 0);
    /* Buffer range for EP1 */
    USBD_SET_EP_BUF_ADDR(EP1, EP1_MSC_BUF_BASE);

    /*****************************************************/
    /* EP2 ==> Bulk IN endpoint, address 2 */
    USBD_CONFIG_EP(EP2, USBD_CFG_EPMODE_IN | MSC_BULK_IN_EP_NUM);
    /* Buffer range for EP2 */
    USBD_SET_EP_BUF_ADDR(EP2, EP2_MSC_BUF_BASE);

    /* EP3 ==> Bulk Out endpoint, address 3 */
    USBD_CONFIG_EP(EP3, USBD_CFG_EPMODE_OUT | MSC_BULK_OUT_EP_NUM);
    /* Buffer range for EP3 */
    USBD_SET_EP_BUF_ADDR(EP3, EP3_MSC_BUF_BASE);

    /* trigger to receive OUT data */
    USBD_SET_PAYLOAD_LEN(EP3, EP3_MSC_MAX_PKT_SIZE);

    /*****************************************************/
    g_u32BulkBuf0 = EP3_MSC_BUF_BASE;
    g_u32BulkBuf1 = EP2_MSC_BUF_BASE;
    ...
    g_STORIF_sFlash.pfnGetInfo(&sStorInfo, NULL);
    g_TotalSectors = (sStorInfo.u32DiskSize * 1024) / UDC_SECTOR_SIZE;
...
void MSCTrans_ClassRequest(void)
{
    ...
    if (buf[0] & 0x80)   /* request data transfer direction */
    {
        ...
        case GET_MAX_LUN:
        {
            ...
            if((buf[4] == s_psUSBDevInfo->gu8ConfigDesc[LEN_CONFIG + 2]) && (buf[2] + buf[3] + buf[6] + buf[7] == 1))
            {
...
//			printf("%s:%d: MSC command %x \n", __FUNCTION__, __LINE__ , g_sCBW.u8OPCode);
            switch (g_sCBW.u8OPCode)
            {
            case UFI_PREVENT_ALLOW_MEDIUM_REMOVAL:
            {
            ...
            case UFI_READ_FORMAT_CAPACITY:
            {
                ...
                if (g_u32Length > 0)
                {
                    if (g_u32Length > EP2_MSC_MAX_PKT_SIZE)
                        g_u8Size = EP2_MSC_MAX_PKT_SIZE;
                    else
                        g_u8Size = g_u32Length;
            ...
            case UFI_READ_10:
            {
                ...
                {
                    /* Set the packet size */
                    if (g_u32BytesInStorageBuf > EP2_MSC_MAX_PKT_SIZE)
                        g_u8Size = EP2_MSC_MAX_PKT_SIZE;
                    else
