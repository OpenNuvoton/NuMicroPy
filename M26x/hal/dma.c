...
static const struct nu_modinit_s dma_modinit = {(uint32_t)PDMA0, PDMA0_MODULE, 0, 0, PDMA0_RST, PDMA0_IRQn, (void *) Handle_PDMA_Irq};
...
{
    uint32_t intsts = PDMA_GET_INT_STATUS(PDMA0);

    // Abort
    if (intsts & PDMA_INTSTS_ABTIF_Msk) {
        uint32_t abtsts = PDMA_GET_ABORT_STS(PDMA0);
        // Clear all Abort flags
        PDMA_CLR_ABORT_FLAG(PDMA0, abtsts);
    ...
    if (intsts & PDMA_INTSTS_TDIF_Msk) {
        uint32_t tdsts = PDMA_GET_TD_STS(PDMA0);
        // Clear all transfer done flags
        PDMA_CLR_TD_FLAG(PDMA0, tdsts);
