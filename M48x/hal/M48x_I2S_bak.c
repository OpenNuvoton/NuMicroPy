...
#if 0
	uint8_t 	sBuff[16] __attribute__((aligned(32)));       /* Working buffer */
    DMA_DESC_T sDMA_DESC_TX[2] __attribute__((aligned(32)));
    DMA_DESC_T sDMA_DESC_RX[2] __attribute__((aligned(32)));
#endif
...
#if 1
	uint8_t 	Buff[96] __attribute__((aligned(256)));       /* Working buffer */
...
//			var->DMA_DESC_TX[1].ctl = ((psTransParam->tx1_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|u32DataWidth|PDMA_SAR_INC|PDMA_DAR_FIX|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			...
			printf("DESC0 offset %x \n", var->DMA_DESC_TX[0].offset);
			printf("DESC1 offset %x \n", var->DMA_DESC_TX[1].offset);
			printf("PDMA->SCATBA %x \n", PDMA->SCATBA);
			printf("sDMA_DESC_TX address  %x \n", &var->DMA_DESC_TX[0]);

#else
//			DMA_DESC_TX[0].ctl = ((psTransParam->tx_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|u32DataWidth|PDMA_SAR_INC|PDMA_DAR_FIX|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
//			PDMA->SCATBA = &sDMA_DESC_TX[0];
			sDMA_DESC_TX[0].ctl = ((psTransParam->tx_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|PDMA_WIDTH_32|PDMA_SAR_INC|PDMA_DAR_FIX|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_TX[0].src = (uint32_t)psTransParam->tx;
			sDMA_DESC_TX[0].dest = (uint32_t)&i2s_base->TXFIFO;
			sDMA_DESC_TX[0].offset = (uint32_t)(&sDMA_DESC_TX[1]) - (PDMA->SCATBA);

//			DMA_DESC_TX[1].ctl = ((psTransParam->tx1_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|u32DataWidth|PDMA_SAR_INC|PDMA_DAR_FIX|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_TX[1].ctl = ((psTransParam->tx1_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|PDMA_WIDTH_32|PDMA_SAR_INC|PDMA_DAR_FIX|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_TX[1].src = (uint32_t)psTransParam->tx1;
			sDMA_DESC_TX[1].dest = (uint32_t)&i2s_base->TXFIFO;
			sDMA_DESC_TX[1].offset = (uint32_t)(&sDMA_DESC_TX[0]) - (PDMA->SCATBA);

			printf("DESC0 offset %x \n", sDMA_DESC_TX[0].offset);
			printf("DESC1 offset %x \n", sDMA_DESC_TX[1].offset);
			printf("PDMA->SCATBA %x \n", PDMA->SCATBA);
			printf("sDMA_DESC_TX address  %x \n", &sDMA_DESC_TX[0]);
#endif
...
#else
//			DMA_DESC_RX[0].ctl = ((psTransParam->rx_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|u32DataWidth|PDMA_SAR_FIX|PDMA_DAR_INC|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_RX[0].ctl = ((psTransParam->rx_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|PDMA_WIDTH_32|PDMA_SAR_FIX|PDMA_DAR_INC|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_RX[0].src = (uint32_t)&i2s_base->RXFIFO;
			sDMA_DESC_RX[0].dest = (uint32_t)psTransParam->rx;
			sDMA_DESC_RX[0].offset = (uint32_t)&sDMA_DESC_RX[1] - (PDMA->SCATBA);

//			DMA_DESC_RX[1].ctl = ((psTransParam->rx1_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|u32DataWidth|PDMA_SAR_FIX|PDMA_DAR_INC|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_RX[1].ctl = ((psTransParam->rx1_length - 1)<<PDMA_DSCT_CTL_TXCNT_Pos)|PDMA_WIDTH_32|PDMA_SAR_FIX|PDMA_DAR_INC|PDMA_REQ_SINGLE|PDMA_OP_SCATTER;
			sDMA_DESC_RX[1].src = (uint32_t)&i2s_base->RXFIFO;
			sDMA_DESC_RX[1].dest = (uint32_t)psTransParam->rx1;
			sDMA_DESC_RX[1].offset = (uint32_t)&sDMA_DESC_RX[0] - (PDMA->SCATBA);
